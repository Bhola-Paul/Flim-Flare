...
import { BrainIcon, MenuIcon, SearchIcon, Ticket, TicketPlus, XIcon } from 'lucide-react'
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'> 
      ...
      {/* menu items */}
        ...
        <Link to="/" onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} className=''>Home</Link>
        <Link to="/movies" onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} className=''>Movies</Link>
        <Link to="/" onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} className=''>Theaters</Link>
        <Link to="/" onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} className=''>Releases</Link>
        <Link to="/favorite" onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} className=''>Favorite</Link>
